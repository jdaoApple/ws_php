layui.define(['layer', 'jquery', 'form', 'table', 'upload', 'laydate', 'authtree'], function (exports) {
    ...
    let hex = {
        ...
        deleteBatchEntity(url, list, done = null) {
            ...
            layer.confirm('您确认要删除吗，这是不可恢复的哦~', {
                btn: ['确认', '取消']
            }, function () {
                $.ajax({
                    url: url,
        ...
        query(elem, table, fields, show = false, values = {}, done = null) {
            let instance = $(elem);
                ...
                switch (item.type) {
                    ...
                    case 'switch':
                        ...
                        d += '<div class="layui-form-item"><input type="hidden" name="' + item.name + '" value="' + (values.hasOwnProperty(item.name) ? values[item.name] : 0) + '">\n' +
                            '                <label class="layui-form-label">' + item.title + required + ' ' + (item.hasOwnProperty("tips") ? '<span style="cursor: pointer;" class="tips-' + item.name + '"><i class="layui-icon" style="color:#cd9898;font-size: 14px;">&#xe607;</i></span>' : '') + '</label>\n' +
                            '                <div class="layui-input-block">\n' +
                            '                    <input class="' + item.name + '" type="checkbox" lay-filter="switch-' + item.name + '" value="0" title="' + item.text + '" ' + (values.hasOwnProperty(item.name) ? (values[item.name] == 1 ? 'checked' : '') : '') + '>\n' +
                            '                </div>\n' +
                            '            </div>';
                        break;
                    case 'select':
                    ...
                    case 'file':
                        ...
                        d += '<div class="layui-form-item" style="' + ((item.hasOwnProperty("hide") && item.hide) ? 'display:none;' : '') + '"><input type="hidden" name="' + item.name + '" value="' + (values.hasOwnProperty(item.name) ? values[item.name] : '') + '">\n' +
                            '    <label class="layui-form-label">' + item.title + required + '</label>\n' +
                            '    <div class="layui-input-block ' + item.name + '">\n' +
                        ...
                        switch (item.type) {
                            case "radio":
                                if (item.hasOwnProperty('dict')) {
                                    ...
                                    this.getDict(item.dict, res => {
                                        res.data.forEach(s => {
                                            instance.append('<input lay-filter="radio-' + item.name + '" name="' + item.name + '" type="radio" value="' + s.id + '" title="' + s.name + '" ' + (values.hasOwnProperty(item.name) ? (values[item.name] == s.id ? 'checked' : '') : '') + ' />');
                                        });
                                        form.render();
                                    });
                                    if (item.hasOwnProperty('change')) {
                                        form.on('radio(radio-' + item.name + ')', function (data) {
                            ...
                            case "select":
                                if (item.hasOwnProperty('data')) {
                                    ...
                                    item.data.forEach((value, index) => {
                                        instance.append(' <option value="' + value.id + '"  ' + (values.hasOwnProperty(item.name) ? (values[item.name] == value.id ? 'selected' : '') : '') + '>' + value.name + '</option>');
                                    });
                                    form.render();
                                }
                                if (item.hasOwnProperty('dict')) {
                            ...
                            case "checkbox":
                                if (item.hasOwnProperty('data')) {
                                    ...
                                    item.data.forEach((value, index) => {
                                        instance.append('<input type="checkbox" ' + (val.indexOf(value.id) !== -1 || val.indexOf(value.id.toString()) !== -1 ? 'checked' : '') + ' value="' + value.id + '" name="' + item.name + '[]" title="' + value.name + '">\n');
                                    });
                                    form.render();
                                }
                                if (item.hasOwnProperty('dict')) {
